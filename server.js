...
async function handleApi(request, response, url) {
    ...
    if (request.method === "POST" && url.pathname === "/api/auth/register") {
        ...
        if (name.length < 2 || !email || password.length < 1) return sendJson(response, 400, { error: "Dados de cadastro invalidos" });
        if (database.users.some(user => user.email === email)) return sendJson(response, 409, { error: "Este e-mail ja esta cadastrado" });
        if (database.users.some(user => user.name.toLowerCase() === name.toLowerCase())) return sendJson(response, 409, { error: "Este nome de usuario ja esta em uso" });
        const user = { id: String(input.id || crypto.randomUUID()), name, email, password, profile: input.profile === "comerciante" ? "comerciante" : "cliente", avatar: input.avatar || null };
        database.users.push(user);
        writeDatabase(database);
        return sendJson(response, 201, { user: { ...user, password: undefined } });
    }
